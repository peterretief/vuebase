...
<script>
import aNewCustomer from './NewCustomer.vue';
...
export default {
  ...
  components: {
    appBottom: Bottom,
    appRight: Right,
    appNewCustomer: aNewCustomer
  },

  computed: {
    ...mapGetters({
        getSelected: types.GET_SELECTED,
        getColumns: types.CUSTOMER_COLUMNS,
        getData: types.GET_ALL_CUSTOMER_DATA,
...
// METHODS
 methods: {
   ...
   openForm: function(){
        this.$store.commit('openCustomer')
...
this.getSelectRow = this.$store.getters["customersById"][key]

this.$store.state.customer_selected_row = this.$store.getters["customersById"][key]
...
// DATA
   data: function () {
       ...
       this.$store.getters.getCustomerColumns.forEach(function (key) {
         sortOrders[key] = 1
       })
       return {
         sortKey: '',
