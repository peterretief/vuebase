...
<script>
import Fragment from '../modal_template/FragmentRental.vue';
import RentalEdit from '../modal_template/RentalEdit.vue';
...
export default {
  ...
  components: {
    ...
    appRentalEdit: RentalEdit
  },

  computed: {
    ...mapGetters({
        getSelected: types.GET_SELECTED,
        getColumns: types.RENTAL_COLUMNS,
        getData: types.GET_ALL_RENTAL_DATA
    }),
...
//console.log(this.$store.getters["rentalsById"][key]);
this.getSelectRow = this.$store.getters["rentalsById"][key]
...
// DATA
   data: function () {
       ...
       this.$store.getters.getRentalColumns.forEach(function (key) {
         sortOrders[key] = 1
       })
       return {
         sortKey: '',
